...
export default CallButton = (props) => {
	const pressHandler = () => {
		if (Platform.OS === "ios") {
	...
	return (
		<View style={[styles.container, props.style, props.wide ? { width: '90%' } : '']}>
			<View style={[styles.wall, props.icon ? styles.clearance : '', { backgroundColor: c.grayLighter }, props.wide ? { width: '100%' } : '']}><Content {...props} hide /></View>
			<View style={[styles.wall, styles.layer, props.icon ? styles.clearance : '', props.wide ? { width: '100%' } : '']}><Content {...props} hide /></View>
			<Pressable
				style={({ pressed }) => [styles.main, pressed ? styles.pressed : '', props.icon ? styles.clearance : '', { backgroundColor: props.color ? props.color : c.grayLighter }]}
...
const styles = {
	...
	text: ({ iconsrc, icon, size, wide, shallow }) => css`
        ...
        margin:  ${Platform.OS === 'android' ? '2px' : '3px'} ${iconsrc || icon ? '14px' : '18px'};
